...
class Progreso(
    ...
    fun mostrarHistoria(pesoCorporal: Double, porcentajeGrasa: Double, medidasCorporales: String, fecha: String) {
        println("Historia del usuario:")
        println("Peso Corporal: $pesoCorporal kg")
        println("Porcentaje de Grasa: $porcentajeGrasa %")
        println("Medidas Corporales: $medidasCorporales")
        println("Fecha: $fecha")
        ...
        if (historialRutinas.isNotEmpty()) {
            ...
            for (rutina in historialRutinas) {
                println("- Nombre de Rutina: ${rutina.nombreRutina}")
                for (ejercicio in rutina.listaActividades) {
                    println("  - Ejercicio: ${ejercicio.nombreEjercicio}")
                }
            }
        }
                ...
                if (usuario != null) {
                    ...
                    while (menuSecundario != 6) {
                        ...
                        when (menuSecundario) {
                            ...
                            5 -> {
                                println("Mostrando Historia:")
                                println("Introduce el Peso Corporal (kg): ")
                                val pesoCorporal = readLine()?.toDoubleOrNull() ?: 0.0
                                println("Introduce el Porcentaje de Grasa (%): ")
                                val porcentajeGrasa = readLine()?.toDoubleOrNull() ?: 0.0
                                println("Introduce las Medidas Corporales: ")
                                val medidasCorporales = readLine() ?: ""
                                println("Introduce la Fecha: ")
                                val fecha = readLine() ?: ""

                                val progresoUsuario = Progreso()
                                progresoUsuario.mostrarHistoria(pesoCorporal, porcentajeGrasa, medidasCorporales, fecha)
                            }
                            6 -> {
